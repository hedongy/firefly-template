/**
 * Created by Tuffy on 2016/12/29.
 */
'use strict';
/*!
...
// 获取变量
    ...
    var _$f = handler.$f = function (option) {
        ...
        ajax(option.templateUrl, function (error, result) {
            if (error == null) {
    ...
    // 模版缓存
    var templateCacheList = new Array();
    _$f.$templateCache = function() {};
    _$f.$templateCache.put = function(url, content) {
        templateCacheList.push({
            url: url,
            content: content
        });
        return _$f.$templateCache;
    };
    _$f.$templateCache.get = function(url) {
        for (var i = 0; i < templateCacheList.length; i++) {
            var item = templateCacheList[i];
            if (item.url === url) {
                return item.content;
            }
        }
        ...
        function doResult() {
            if (xmlHttpReg.readyState == 4) {//4代表执行完成
                if (xmlHttpReg.status == 200) {//200代表执行成功
                    ...
                    if (callback) {
                        callback(null, xmlHttpReg.responseText);
                    }
                } else {
                    if (callback) {
                        callback({'errorCode': xmlHttpReg.status});
                    }
                }
            }
        };
